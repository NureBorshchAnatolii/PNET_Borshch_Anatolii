...
public class TestAttemptRepository : ITestAttemptRepository
{
    ...
    public TestAttemptRepository(ApplicationDbContext db)
    {
        _db = db;
    }
    ...
    {
        return await _db.TestAttempts.Where(a => a.UserId == userId)
            .Include(a => a.Test).ToListAsync();
    }

    public async Task<int> CountAttemptsAsync(Guid userId, Guid testId)
    {
        return await _db.TestAttempts.CountAsync(a =>
    ...
    {
        return await _db.TestAttempts.FirstOrDefaultAsync(a =>
            a.UserId == userId && a.TestId == testId &&
